...
async function handleAdminCommands(msg, client, OWNER_IDS, loadData, saveData, USERS_FILE) {
    ...
    collector.on('collect', async (interaction) => {
        ...
        if (id === 'amenu_resetall') {
            ...
            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('amenu_resetall_confirm').setLabel('✅ 全部リセット').setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId('amenu_econ').setLabel('❌ キャンセル').setStyle(ButtonStyle.Secondary)
            );
            const row2 = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('amenu_resetjson_econ').setLabel('econ').setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId('amenu_resetjson_corp').setLabel('corp').setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId('amenu_resetjson_crypto').setLabel('crypto').setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId('amenu_resetjson_auctions').setLabel('auctions').setStyle(ButtonStyle.Secondary),
                new ButtonBuilder().setCustomId('amenu_resetjson_loans').setLabel('loans').setStyle(ButtonStyle.Secondary)
            );
            const row3 = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('amenu_resetjson_trades').setLabel('trades').setStyle(ButtonStyle.Secondary),
                new ButtonBuilder().setCustomId('amenu_resetjson_birthday').setLabel('birthday').setStyle(ButtonStyle.Secondary)
            );
            return interaction.update({ embeds: [embed], components: [row, row2, row3] });
        }

        if (id.startsWith('amenu_resetjson_')) {
            const target = id.replace('amenu_resetjson_', '');
            const fs = require('fs'), path = require('path');
            const p = path.join(__dirname, 'data', `${target}.json`);
            if (!fs.existsSync(p)) return interaction.reply({ content: `❌ ${target}.json が見つかりません。`, flags: MessageFlags.Ephemeral });
            fs.writeFileSync(p, JSON.stringify({}, null, 4));
            return interaction.reply({ content: `✅ **${target}.json** をリセットしました。`, flags: MessageFlags.Ephemeral });
        }
...
async function handleAdminModal(interaction, client, OWNER_IDS) {
...
module.exports = { handleAdminCommands: handleAdminCommands, handleAdminModal };
